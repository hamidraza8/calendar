...
let dayClassName=(currentDate===calenderDate)?"calendar-day is-active":"calendar-day"
dayClassName=!isCurrentMonthDate(date)?dayClassName+" disabled-day":dayClassName;

let eventClassName=(currentDate===calenderDate)?"calendar-event is-active":"calendar-event"
...
const EventDetailPopupContent = (
  <div>
    ...
    { <div className='event-detail-content'>On <b>{date.format("MMM Do YY")} </b> From <b>{fromEventTime}</b> To <b>{toEventTime}</b></div> }
...
{dateDisplay}
<Popover content={EventDetailPopupContent}  placement="right"  title="Calendar">
<div className={eventClassName}>
 
  {eventName}
  {(eventName!==undefined)?(<><div>{fromEventTime} - {toEventTime}</div></>):<></>}
